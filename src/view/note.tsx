...
export function Note({ note, deleteNote, editNote }: NoteProps) {
  ...
  return (
    ...
    >
      <Dialog.Trigger className="relative flex flex-col gap-3 overflow-hidden rounded-4xl bg-slate-700 p-5 outline-none hover:ring hover:ring-slate-600 focus-visible:ring focus-visible:ring-lime-400">
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="md:rounded-4xl fixed inset-0 flex w-full flex-col bg-slate-700 outline-none md:inset-auto md:left-1/2 md:top-1/2 md:h-[60vh] md:max-w-[640px] md:-translate-x-1/2 md:-translate-y-1/2">
          <Dialog.Close className="absolute right-2 top-2 rounded-full bg-slate-800 p-4 text-slate-400 outline-none hover:text-red-400 focus-visible:text-red-400 focus-visible:ring md:-right-2 md:-top-2">
            <X className="size-5" />
          </Dialog.Close>
          ...
          <div className="flex flex-1 flex-col gap-3 p-5">
            ...
            {shouldShowOnboarding ? (
              <textarea
                autoFocus
                className="scrollbar-hide flex-1 resize-none rounded-4xl bg-slate-600/50 p-2 text-lg leading-6 text-slate-200 outline-none transition focus:shadow-2xl"
          ...
          <div className="grid grid-cols-2 gap-x-4 px-2 pb-2 md:gap-x-0">
            {shouldShowOnboarding ? (
              <>
                <Button
                  type="button"
